...
import image from './no_search_result.jpg'
import './index.css'
// Tabs List
// git remote add origin https://github.com/Tamilarasan117/app-store.git
const tabsList = [
...
class AppStore extends Component {
  ...
  // Method for getting active tab list
  getActiveTabId = () => {
    const {activeTabItem} = this.state
    const filterTabItemApps = appsList.filter(eachApp => (
      activeTabItem.toLowerCase()
        ===
      eachApp.category.toLowerCase()
    ))
    return filterTabItemApps
  }
  // Method for getting active tab id
  updateActiveTabItem = (tabId) => {
    this.setState({activeTabItem: tabId})
  }
  // Method for retriving search result
  onSearchApp = (event) => {
    this.setState({searchInput: event.target.value})
  }

  render() {
    const filtredTabItemApps = this.getActiveTabId()
    const {searchInput,activeTabItem} = this.state
    // Callback Function for filtering searched apps
    const filtredApps = filtredTabItemApps.filter(eachApp => (
      eachApp.appName.toLowerCase().includes(searchInput.toLowerCase())
    ))
    if(filtredApps.length) {
      // Display filtred apps
      return (
        <Fragment>
          <div className="app-store-container">
            <div className="container">
              ...
              {
                // Callback function for getting tab lists
              }
              <ul className="tab-item-container">
                {
                  tabsList.map(eachTab => (
                    <TabItem 
                      ...
                      active={activeTabItem === eachTab.tabId}
                    />
                  ))
                }
              </ul>
              {
                // Callback function for getting app lists
              }
              <ul className="app-items-container">
                {
                  filtredApps.map(eachApp => (
                    <AppItem
                      appItemDetails={eachApp}
                      key={eachApp.appId}
                    />
                  ))
                }
        ...
        <div className="app-store-container">
          <div className="container">
            ...
            <div className="empty-result">
              <h1 className="no-result">There is no such search app</h1>
            </div>
          </div>
        </div>
      </Fragment>
